                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="fa fa-cog fa-fw"></i> 配置《<?php echo ($remark); ?>》角色权限</h3>
                            <div class="text-right"><a href="<?php echo U('listAll');?>">角色列表</a></div>
                        </div>
                        <form action="<?php echo U('accessHandler', 'id='.$id);?>" method="post">
                        <div class="panel-body">
                            <?php if(is_array($items)): foreach($items as $key=>$vo): ?><div class="panel panel-default">
                                    <div class="panel-heading">
                                        <?php if($vo['status'] == 1): ?><input type="checkbox" name="node[]" value="<?php echo ($vo['id']); ?>" checked="checked">
                                        <?php else: ?>
                                            <input type="checkbox" name="node[]" value="<?php echo ($vo['id']); ?>"><?php endif; ?>
                                        <?php echo ($vo["title"]); ?>
                                    </div>
                                    <div class="panel-body">
                                        <?php if(is_array($vo['child'])): foreach($vo['child'] as $k=>$v): ?><div class="panel panel-default">
                                            <div class="panel-heading">
                                            <?php if($v['status'] == 1): ?><input type="checkbox" name="node[]" value="<?php echo ($v['id']); ?>" checked="checked">
                                            <?php else: ?>
                                                <input type="checkbox" name="node[]" value="<?php echo ($v['id']); ?>"><?php endif; ?>
                                            <?php echo ($v["title"]); ?>
                                            </div>
                                            <div class="panel-body">
                                                <?php if(is_array($v['child'])): foreach($v['child'] as $k1=>$v1): ?><label class="checkbox-inline">
                                                    <?php if($v1['status'] == 1): ?><input type="checkbox" name="node[]" value="<?php echo ($v1['id']); ?>" checked="checked"><?php echo ($v1["title"]); ?>
                                                    <?php else: ?>
                                                    <input type="checkbox" name="node[]" value="<?php echo ($v1['id']); ?>"><?php echo ($v1["title"]); endif; ?>
                                                </label><?php endforeach; endif; ?>
                                            </div>
                                        </div><?php endforeach; endif; ?>
                                    </div>
                                </div><?php endforeach; endif; ?>
                            <?php if(!empty($items)): ?><button type="submit" class="btn btn-default">提交</button>
                            <button type="reset" class="btn btn-default">重置</button>
                            <?php else: ?>
                            没有添加任何权限，请<a href="<?php echo U('Node/editApp');?>">添加权限节点名称</a>。<?php endif; ?>
                        </div>
                        </form>
                    </div>
                </div>
