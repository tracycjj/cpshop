...
<div class="wrapper">
  ...
  <header class="site-header">
    <div class="cart-summary accent-text">
      ...
      <p class="large--left medium-down--hide" <?php if(!$user): ?>style="display:none;"<?php endif; ?>>
          登录账号 <a href="/User/index"><?php echo ($user["username"]); ?></a> · <a href="/Login/logout" id="customer_logout_link">退出</a>
      </p>
...
<div id="content" role="main">
      <div id="customer-accounts" class="content customer">
  ...
  <!-- Create Customer -->
        <div id="create-customer">
          ...
          <div class="boxed-form">
            <form method="post" action="/Login/registerHandler" id="create_customer" accept-charset="UTF-8" onsubmit="return checkregis()">
              <div id="first_name" class="clearfix form-row">
                <label for="first_name" class="login">用户名</label>
                <input type="text" name="username" id="first_name" class="styled-input" placeholder="用户名" value="" size="30">
              </div>

              <div id="email" class="clearfix form-row">
                <label for="email" class="login">邮箱</label>
                <input type="email" name="email" id="email" class="styled-input" placeholder="邮箱" value="" size="30">
              </div>

              <div id="password" class="clearfix form-row">
                <label for="password" class="login">密码</label>
                <input type="password" name="password" id="password" class="password styled-input" value="" size="30">
              </div>
              <div id="password" class="clearfix form-row">
                <label for="password" class="login">重复密码</label>
                <input type="password" name="repassword" id="repassword" class="password styled-input" value="" size="30">
              </div>
...
<script type="text/javascript">
  ...
  function checkregis(){
    ...
    if(!username){
      alert("用户名不能为空!")
      $("input[name=username]").css({"border":"1px solid #fb8077"});
      $("input[name=username]").focus(); return false;
    }
    if(username == 'admin'){
      alert("用户名不合法!")
      $("input[name=username]").css({"border":"1px solid #fb8077"});
    ...
    if(!/[a-z0-9A-Z]+@[a-z0-9A-Z]+\.[a-z]+/.test(email)){
      alert("邮箱不合法!");
      $("input[name=email]").css({"border":"1px solid #fb8077"});
      $("input[name=email]").focus(); return false;
    }
    if(password.length<6){
      alert("密码长度过短!");
      $("input[name=password]").css({"border":"1px solid #fb8077"});
      $("input[name=password]").focus(); return false;
    }
    if(repassword != password){
      alert("两次输入密码不一致!");
      $("input[name=repassword]").css({"border":"1px solid #fb8077"});
      $("input[name=repassword]").focus(); return false;
    }
    return true;
  }
</script>  
