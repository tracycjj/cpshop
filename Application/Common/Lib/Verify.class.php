...
namespace Common\Lib;
    class Verify{
        ...
        private $fonturl;
        private $session;
        function __construct($width = 120,$height = 30,$counts = 4,$distrubcode="1235467890qwertyuipkjhgfdaszxcvbnm",$fonturl="Fibography_PersonalUse.ttf"){
            $this->width=$width;
            $this->height=$height;
            $this->counts=$counts;
            $this->distrubcode=$distrubcode;
            $this->fonturl=dirname(__FILE__)."/".$fonturl;
         ...
         function imageout($id){
            ...
            $_SESSION['code_'.$id]=$this->session;
            ImageGIF($im);
            ImageDestroy($im); 
        }
